package sudokufromgithubaccountharshpai;
...
public class Sudoku {
    ...
    private final HashSet<Integer> allValues = new HashSet<>();
    ...
    private class Spot implements Comparable<Spot> {
        ...
        /**
         * Calculates the number of possible values that can be assigned to this
         * spot, if it is empty. Else returns 0.
         *
         */
        ...
        /**
         * Returns a set containing integers that cannot be assigned to "this"
         * spot. Our strategy is to build a set of values present in the same
        ...
        private Set<Integer> getNonAssignableValues() {
            ...
            for (int i = 0; i < SIZE; i++) {
                if (grid[row][i] > 0) {
                    nonAssignableValues.add(grid[row][i]);
                }
            }
            ...
            for (int i = 0; i < SIZE; i++) {
                if (grid[i][col] > 0) {
                    nonAssignableValues.add(grid[i][col]);
                }
            }
            ...
            for (int i = 0; i < PART; i++) {
                for (int j = 0; j < PART; j++) {
                    if (grid[partInitRow + i][partInitCol + j] > 0) {
                        nonAssignableValues.add(grid[partInitRow + i][partInitCol + j]);
                    }
                }
            }
        ...
        /**
         * Returns a set containing integers that can be assigned to "this"
         * spot.
         *
         */
        ...
        /**
         * Compares the spot to the given object for sort order. (This is the
         * standard sorting override, implemented here since we implement the
         * "Comparable" interface.) Increasing order by assignableSetSize. This
         * is the Java 5 version, generic for Comparable<Spot>, so the arg is
         * type Spot.
         *
    ...
    public static final int[][] hardGridV2 = Sudoku.stringsToGrid(
            "0 0 0 1 6 2 4 8 9",
            "0 0 0 4 9 3 5 2 7 ",
            "0 0 0 7 8 5 1 6 3 ",
            "4 9 3 8 5 7 6 1 2 ",
    ...
    public static int[] stringToInts(String string) {
        ...
        //TODO: 1) find out why we have to copy the array "a" here instead of just returning it. My guess is that if we return array "a",
        //we are simply returning a pointer to it. But even if we return array "result", are we not simply returning a pointer to array "result"
        //as well? If in both instance, we are returning a pointer, then what is the difference anyway? Require some serious thinking 
    ...
    /**
     * Renders the Sudoku as a String made of 9 lines that shows the rows of the
     * grid, with each number preceded by a space.
     *
     */
    ...
    /**
     * Private helper method that renders the Sudoku as a String made of 9 lines
     * that shows the rows of the grid, with each number preceded by a space.
     *
     */
    ...
    int numberOfTimesRecursiveSolveMethodIsCalled = 0;

    /**
     * This is the method that actually solves the sudoku puzzle using classic
     * recursive backtracking algorithm.
     *
     */
    private void rescursiveSolve(int index) {
        numberOfTimesRecursiveSolveMethodIsCalled++;
        System.out.println(numberOfTimesRecursiveSolveMethodIsCalled);
        if (numSolutions == 100) {
            return;
        }
    ...
    /**
     * Returns an ArrayList containing empty spots in the sudoku grid. The spots
     * are sorted by assignableSetSize in increasing order.
     *
     */
    ...
    /**
     * Returns the text form of the first solution found. If no solution was
